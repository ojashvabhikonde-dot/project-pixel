...
import { X, Lock, Mail, User, ShieldCheck, Camera, Upload, Trash2, Sparkles, CheckCircle2 } from 'lucide-react';
...
export default function LoginModal({ isOpen, onClose, onSuccess }: LoginModalProps) {
  ...
  const [specialization, setSpecialization] = useState('Lead Cinematographer');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = isRegister 
      ? {
          ...
          password,
          role,
          specialization: isCrew ? (specialization || 'Visual Creator') : undefined,
          avatarUrl: isCrew ? avatarUrl : undefined,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4 overflow-y-auto">
      <div className="relative w-full max-w-md my-8 overflow-hidden rounded-xl glass-panel border border-white/10 p-6 sm:p-8 text-white shadow-2xl">
        {/* Background glow */}
        ...
        <div className="flex flex-col items-center mb-6 text-center">
          ...
          <p className="text-xs text-zinc-400 mt-1 max-w-xs leading-relaxed">
            {isRegister 
              ? 'Register your profile to be featured on the crew list and upload frames' 
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="pt-2 border-t border-zinc-800/60 space-y-3">
                  <div>
                    <label className="block text-[10px] font-bold text-primary uppercase tracking-widest mb-1.5 font-mono flex items-center justify-between">
                      <span>Crew Profile Photo (Required for Crew)</span>
                    ...
                    <div className="flex items-center space-x-3 bg-zinc-950/80 p-2.5 rounded-lg border border-zinc-800">
                      {avatarUrl ? (
                        <div className="relative h-14 w-14 rounded-lg overflow-hidden border border-primary shrink-0 shadow-md">
                          <img src={avatarUrl} alt="Crew Avatar Preview" className="h-full w-full object-cover" />
                        </div>
                      ) : (
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-zinc-400 uppercase tracking-wider mb-1 font-mono">Specialization / Role</label>
                    <input
                      type="text"
                      placeholder="e.g. Lead Cinematographer, Street Specialist, Editor"
          ...
          >
            {loading ? 'Processing...' : isRegister ? 'Register & Join Crew' : 'Sign In to Pixela'}
          </button>
        </form>
          ...
          {isRegister ? (
            <p>
              ...
              <button 
                ...
                className="text-primary hover:underline focus:outline-none ml-1 font-bold"
              >
                Sign In
              </button>
            </p>
